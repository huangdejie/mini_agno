...
agent = Agent(
    model=OpenAIModel(),
    tools=[query_temperature, query_humidity],
    instructions="你是一个 helpful的助手，根据用户输入的烤房名称，查询烤房的温度和湿度，并给出分析。",
)
...
def chat(request: RunRequest) -> RunResponse:
    return RunResponse(
        content=agent.run(request.message, request.session_id, request.user_id)
    )
