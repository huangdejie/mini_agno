...
class Team:
    ...
    leader: Agent = field(init=False) # coordinate 时在 __post_init__ 中初始化,不需要显式传入
    ...
    def __post_init__(self):
        ...
        if self.mode == "sequential":
            ...
            self.leader = Agent(model=self.leader_model, tools=[self._make_delegate_tool()], instructions=self._build_leader_instructions())
        else:
    ...
    def _validate_coordinate(self):
        ...
        for m in self.members:
            ...
            if not m.description:
                raise ValueError(f"coordinate mode: member '{m.name}' must have a description")
            names.append(m.name)
    ...
    def _build_leader_instructions(self) -> str:
        ...
        lines.append("使用 `delegate_task_to_member(member_id, task)` 委派任务。委派后根据结果决定下一步，最终给出完整回答。")
        return "\n".join(lines)
