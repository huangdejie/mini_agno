...
def test_session_psersists_across_agent_instances(tmp_path):
    ...
    # 第一个 Agent 实例写入
    a1 = Agent(model=MockModel(id="m1", response_list=[ModelResponse(content="你叫张三")]), tools=[], db=db)
    a1.run("我叫张三", session_id="s1")

    # 第二个全新实例读取
    a2 = Agent(model=MockModel(id="m2", response_list=[ModelResponse(content="你叫张三")]), tools=[], db=db)
