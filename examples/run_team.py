...
def run_team():
    ...
    temp_agent = Agent(model=model, name="temperatureAgent", tools=[query_temperature], description="你是一个善于根据工具查询烤房温度的助手")

    analysis_agent = Agent(model=model, name="analysisAgent",tools=[],description="你是一个善于根据烤房温度进行分析的助手（不要依赖于其他技术标准仅依赖以下规则）.规则如下：如果烤房小于40度，则建议提升火力；如果烤房温度大于等于40度但小于50度，则建议保持火力；如果烤房温度大于等于50度，则建议降低火力。")
    members.append(analysis_agent)
    members.append(temp_agent)

    team = Team(members=members, leader_model=model, mode="coordinate")
    a = team.run("帮我分析河南平顶山001烤房的温度情况，并给出当前火力操作建议，不要依赖于其他行业标准，并说出用到了哪些工具")
    print(a)
